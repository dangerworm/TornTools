...
import { fetchThemes, postThemeDefinition, postUserThemeSelection } from '../lib/dotnetapi'
...
const createMuiThemeFromDefinition = (definition: ThemeDefinition) => {
  ...
  return createTheme({
    palette: {
      ...
      primary: { main: definition.primaryColor },
      secondary: { main: definition.secondaryColor },
      ...(isDark && {
        background: {
          default: '#0c0e12',
          paper: '#131519',
...
export const ThemeProvider = ({ children }: ThemeProviderProps) => {
  ...
  const [availableThemes, setAvailableThemes] = useState<ThemeDefinition[]>(baseThemes)
  ...
  const muiTheme = useMemo(() => createMuiThemeFromDefinition(resolvedTheme), [resolvedTheme])
  ...
  const refreshThemes = useCallback(async () => {
    ...
    try {
      ...
      const preferredThemeId = dotNetUserDetails?.preferredThemeId ?? selectedThemeId
      const preferredTheme = themes.find((t) => t.id === preferredThemeId)
      const fallbackTheme =
        themes.find((t) => t.name.toLowerCase().includes('light')) ?? themes[0] ?? baseThemes[0]
      const nextTheme = preferredTheme ?? fallbackTheme
      setSelectedThemeId(nextTheme.id)
      localStorage.setItem(LOCAL_STORAGE_SELECTED_THEME_ID, nextTheme.id.toString())
    } catch (e) {
  ...
  const selectTheme = useCallback(
    async (themeId: number | null) => {
      const targetTheme = availableThemes.find((t) => t.id === themeId) ?? availableThemes[0]
      setSelectedThemeId(targetTheme?.id ?? null)
      if (targetTheme?.id !== undefined && targetTheme?.id !== null) {
        localStorage.setItem(LOCAL_STORAGE_SELECTED_THEME_ID, targetTheme.id.toString())
      } else {
      ...
      if (dotNetUserDetails?.id && targetTheme?.id) {
        try {
          const updatedUser = await postUserThemeSelection(dotNetUserDetails.id, targetTheme.id)
          if (updatedUser) {
